...
public class N3 {
	...
	public static void main(String[] args) throws PlatoNoExiste, EleccionIncorrecta {
		...
		if(!Arrays.asList(platos).contains(eleccion.get(0))) {
			eleccion.remove(0);
			throw new PlatoNoExiste("No existe ese plato, eliga un plato válido");
		...
		System.out.println("¿Quieres añadir algún plato más? Si/No");
		String añadir;
		añadir = sc.nextLine();
		try { 
		
		if(añadir.contains("Si")||añadir.contains("No")) {
			System.out.println("----------------");
		} 
		else{
			throw new EleccionIncorrecta("Introduce una opción válida (Si/No)1");
			} 
		...
		while(añadir.contains("Si")) {
			try {

			System.out.println("¿Qué deseas añadir a tu pedido?: ");
			eleccion.add(sc.nextLine());
			...
			try { 
				...
				if(añadir.contains("Si")||añadir.contains("No")) {
					...
					throw new EleccionIncorrecta("Introduce una opción válida (Si/No)");
					} 
